...
1. Регистрируемся на сервисе GitHub (https://github.com/) и создаем свой аккаунт.

2. В своем аккаунте на GitHub создаем копию репозитория из другого аккаунта с помощью кнопки **Fork**.

3. Клонируем репозиторий на свой компьютер с помощью команды `git clone <URL удаленного репозитория>`. Командой `git remote -v` проверяем, что привязано из удаленного репозитория.

4. Создаем новую ветку, используя команду `git branch`.

5. В новой ветке работаем, вносим изменения, делаем коммиты.

6. Отправляем изменения на GitHub командой `git push`. Git не сможет выполнить эту команду, так как нашей ветки не окажется в удаленном репозитории. Git предложит выполнить команду `git push --set-upstream origin`, тем самым мы свяжем локальную и удаленную ветки.

7. После этого мы идем на сайт GitHub, где в нашем репозитории должна появиться зеленая кнопка **Compare & pull request**. Нажимаем на нее. Если кнопка не появилась делаем вручную. 